...
public class SecureAuthentication implements HttpAuthenticationMechanism, Serializable {
    @Inject IdentityStoreHandler handler;       //imported from external source directly
    CredentialValidationResult result;
    AuthenticationStatus status;
    @Inject TokenProvider tokenProvider;        //imported file that we have written
    ...
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext ctx) throws AuthenticationException {
        ...
        try {
            if (request.getRequestURI().contains("Logout")) {
                ...
                {
                    response.sendRedirect("/DoubtCart/App.xhtml?faces-redirect=true");
                }
                else
                {
                    response.sendRedirect("App.xhtml?faces-redirect=true");
                }
...
//                
                ...
                if (result.getStatus() == Status.VALID) {
                    ...
                    if (result.getCallerGroups().contains("Admin")) {
                        ...
                        //System.out.println("Hii I'm Admin..");
                    ...
                    if (result.getCallerGroups().contains("User")) {
                     ...
                     if (result.getCallerGroups().contains("Moderator")) {
                        request.getRequestDispatcher("Home.xhtml?faces-redirect=true").forward(request, response);
                    }
                    ...
                    KeepRecord.setErrorStatus("Either Username or Password is wrong !!  (Note If Your Account is Block You can not Login)" );
                    response.sendRedirect("App.xhtml?faces-redirect=true");
            ...
            if (KeepRecord.getToken() != null) {
                ...
                if(request.getRequestURI().contains("Adminside") && result.getCallerGroups().contains("User"))
                {
                    response.sendError(403);
                }
//               else if(request.getRequestURI().contains("") && result.getCallerGroups().contains("Admin"))
//               {
//                   response.sendError(403);
//               }
